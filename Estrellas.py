...
Ns=np.linspace(0.91,0.1,40) #sequence for gg=0 hh=0 -   Ns IS THE ARRAY OF VALUES FOR THE MINIMUM OF THE LAPSE FUNCTION
#Ns=np.linspace(0.91,0.99,20) #sequence for gg=0 hh=0 + (+ means the part of the solutions that connects with omega=1)

#Ns=np.linspace(0.91,0.45,50) #sequence for gg=-10 hh=0 -
#Ns=np.linspace(0.91,0.99,20) #sequence for gg=-10 hh=0 +
#Ns=np.linspace(0.45,0.442,10) #sequence for gg=-10 hh=0 (continuation of -)
...
for i in np.arange(len(Ns)):
    ...
    datafile=solution_folder+"/lapse_"+"{:.2e}".format(Ns[i-1])+".dat"
    ...
    #############################
        ...
        residuoomega=pch.Chebyshev(vn1_f)(0.0)-N00
    ...
    np.savetxt(solution_folder+"/lapse_"+"{:.2e}".format(N00)+".dat", Un)
